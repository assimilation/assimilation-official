...
class DispatchTarget(object):
    ...
    def dispatch(self, origaddr, frameset):
        ...
        print(
            "Received unhandled FrameSet of type [%d:%s] from [%s]"
...
class DispatchSTARTUP(DispatchTarget):
    ...
    def dispatch(self, origaddr, frameset):
        ...
        drone = self.droneinfo.add(
            sysname, "STARTUP packet", port=origaddr.port(), primary_ip_addr=str(origaddr)
        )
        ...
        if keyid is None or pubkey is None:
            ...
            if drone.key_id == "":
                if not keyid.startswith(sysname + "@@"):
                    CMAdb.log.warning(
                        "Drone %s wants to register with key_id %s -- permitted.", sysname, keyid
                    )
        ...
        CMAdb.net_transaction.add_packet(
            origaddr, FrameSetTypes.SETCONFIG, (str(self.config),), FrameTypes.CONFIGJSON
        )
    ...
    def validate_source_ip(sysname, origaddr, jsobj, listenaddr):
        ...
        if not match:
            CMAdb.log.warning(
                "Drone %s sent STARTUP packet with NATted source address (%s)" % (sysname, origaddr)
            )
            isNAT = True
            if primaryip is not None:
                if CMAdb.running_under_docker():
                    CMAdb.log.warning(
                        "Drone %s STARTUP orig address assumed to be (%s)" % (sysname, primaryip)
                    )
...
class DispatchHBMARTIAN(DispatchTarget):
    ...
    def dispatch(self, origaddr, frameset):
        ...
        if CMAdb.TheOneRing.are_partners(reporter, martiansrc):
            if CMAdb.debug:
                CMAdb.log.debug(
                    "DispatchHBMARTIAN: Ignoring msg from %s about %s" % (reporter, martiansrc)
                )
        else:
            if CMAdb.debug:
                CMAdb.log.info(
...
class DispatchSWDISCOVER(DispatchTarget):
    ...
    def dispatch(self, origaddr, frameset):
        ...
        if CMAdb.debug:
            CMAdb.log.debug(
                "DispatchSWDISCOVER: received [%s] FrameSet from [%s]"
                % (FrameSetTypes.get(fstype)[0], str(origaddr))
            )
        wallclock = None
        interface = None
        ...
        for frame in frameset.iter():
            ...
            if frametype == FrameTypes.HOSTNAME:
                ...
                switchjson = pySwitchDiscovery.decode_discovery(
                    designation, interface, instance, wallclock, pktstart, pktend
                )
                if CMAdb.debug:
                    CMAdb.log.debug(
                        "Got Link discovery info from %s: %s" % (interface, str(switchjson))
                    )
...
class DispatchRSCOPREPLY(DispatchTarget):
    ...
    def dispatch(self, origaddr, frameset):
        ...
        if CMAdb.debug:
            CMAdb.log.debug(
                "DispatchRSCOPREPLY: received [%s] FrameSet from [%s]"
                % (FrameSetTypes.get(fstype)[0], str(origaddr))
            )
        ...
        for frame in frameset.iter():
            ...
            if frametype == FrameTypes.RSCJSONREPLY:
                ...
                return
        CMAdb.log.critical(
            "RSCOPREPLY message from %s did not have a RSCJSONREPLY field" % (str(origaddr))
        )
